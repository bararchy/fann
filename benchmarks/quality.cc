							   ...
							   unsigned int seconds_of_training, double seconds_between_reports)
{
	...
	while(total_elapsed < (double) seconds_of_training)
	{
		...
		while(elapsed < (double) seconds_between_reports)
		{
			...
			{
				ann->set_input((real*)train_data->input[i]);
				ann->train_on((real*)train_data->output[i]);
			}
		...
		{
			ann->set_input((real*)train_data->input[i]);
			output = (fann_type*)ann->query_output();
		...
		{
			ann->set_input((real*)test_data->input[i]);
			output = (fann_type*) ann->query_output();
...
*/
	...
	while(total_elapsed < (double) seconds_of_training)
	{
		...
		report_error(total_elapsed, epochs, test_out, test_collector, fann_get_total_neurons(ann));
		...
		report_error(total_elapsed, epochs, train_out, train_collector, fann_get_total_neurons(ann));

		fprintf(stderr, "secs: %8.2f, train: %8.6f (%4d), test: %8.6f (%4d), epochs: %5d, neurons: %d\r",
				total_elapsed, train_error, train_bit_fail, test_error, test_bit_fail, epochs, fann_get_total_neurons(ann));
	}
