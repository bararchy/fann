...
#ifdef SSE
void performance_benchmark_sse_fann(bool stepwise, FILE * out, fann_type * input,
								unsigned int num_neurons, unsigned int seconds_per_test)
{
	unsigned int i, total_connections;
	fann_type *output;

	struct fann *ann = fann_create_standard_sse_batch_sigmoid(
			4,
								   num_neurons, num_neurons, num_neurons, num_neurons);
...
#ifdef SSE
		else if(strcmp(argv[1], "fann_sse") == 0)
		{
			performance_benchmark_sse_fann(true, out, input, num_neurons, seconds_per_test);
		}
#endif
