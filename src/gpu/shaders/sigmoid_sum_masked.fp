...
//iterate over the input vector, texel by texel
  for (float i=i_delta/2.0; i<1.0; i+=i_delta){
	...
	masked_weight = texture2D(weights, weight_tuple.xy) * texture2D(mask, weight_tuple.xy);
	sum.r += dot(input, masked_weight);
	masked_weight = texture2D(weights, weight_tuple.zw) * texture2D(mask, weight_tuple.zw);
	sum.g += dot(input, masked_weight);
	masked_weight = texture2D(weights, weight_tuple2.xy) * texture2D(mask, weight_tuple2.xy);
	sum.b += dot(input, masked_weight);
	masked_weight = texture2D(weights, weight_tuple2.zw) * texture2D(mask, weight_tuple2.zw);
	sum.a += dot(input, masked_weight);
  } 
  //return the sigmoid of the sum
  vec4 sigmoid = (1.0/(1.0 + exp(-2.0 * sum)));
